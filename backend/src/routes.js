...
router.get('/applications/stats', (req, res) => {
  ...
  res.json({ ok: true, data: { total, byStatus, byRole, overdue, conflict, archived, evidenceBad } });
});
...
router.post('/applications', (req, res) => {
  ...
  const { company_name, credit_line, applicant, contact_phone, business_type } = req.body || {};
  ...
  const info = db.prepare(`
    INSERT INTO credit_applications (app_no, company_name, credit_line, applicant, contact_phone, business_type, status, current_handler_role, current_handler_id, version, evidence_status, created_by)
    VALUES (?, ?, ?, ?, ?, ?, 'draft', 'registrar', ?, 1, 'incomplete', ?)
  `).run(app_no, company_name, parseFloat(credit_line) || 0, applicant, contact_phone, business_type, user.id, user.id);
  ...
  const insEv = db.prepare('INSERT INTO evidence_items (app_id, evidence_type, evidence_name, is_required) VALUES (?, ?, ?, ?)');
  tpls.forEach(t => insEv.run(appId, t[0], t[1], t[2]));
  logOperation({ app_id: appId, user_id: user.id, user_name: user.name, user_role: user.role, action: 'create', old_status: null, new_status: 'draft', version_from: 1, version_to: 1, ip: req.ip });
  res.json({ ok: true, data: { id: appId, app_no } });
});
...
router.put('/applications/:id/evidence/:eid', (req, res) => {
  ...
  if (!app) return res.status(404).json({ ok: false, msg: '申请不存在' });
  if (user.role !== 'registrar') return res.status(403).json({ ok: false, msg: '仅登记员可更新证据' });
  const { is_submitted, file_name, remark } = req.body || {};
  db.prepare('UPDATE evidence_items SET is_submitted = ?, file_name = ?, remark = ?, submit_time = datetime("now","localtime") WHERE id = ? AND app_id = ?').run(is_submitted ? 1 : 0, file_name, remark, eid, id);
  const ev = validators.checkEvidence(id, false);
  db.prepare('UPDATE credit_applications SET evidence_status = ?, updated_at = datetime("now","localtime") WHERE id = ?').run(ev.status, id);
  logOperation({ app_id: id, user_id: user.id, user_name: user.name, user_role: user.role, action: 'evidence_update', old_status: app.status, new_status: app.status, evidence_check: ev.status, version_from: app.version, version_to: app.version, ip: req.ip, extra: JSON.stringify({ evidence_id: eid, is_submitted }) });
  res.json({ ok: true, data: ev });
});

function updateProcessNode(appId, nodeType, version, handler, opinion, result) {
  const node = db.prepare('SELECT * FROM process_nodes WHERE app_id = ? AND node_type = ? AND version = ? AND status IN ("processing","pending") ORDER BY id DESC LIMIT 1').get(appId, nodeType, version);
  if (node) {
    db.prepare('UPDATE process_nodes SET handler_id = ?, handler_role = ?, handler_name = ?, opinion = ?, result = ?, status = "completed", end_time = datetime("now","localtime"), duration_seconds = CAST((julianday("now","localtime") - julianday(start_time)) * 86400 AS INTEGER) WHERE id = ?').run(handler.id, handler.role, handler.name, opinion, result, node.id);
  }
}

function addProcessNode(appId, nodeType, order, version, handler, status) {
  const lastNode = db.prepare('SELECT MAX(node_order) as max_o FROM process_nodes WHERE app_id = ?').get(appId);
  const o = order || (lastNode.max_o ? lastNode.max_o + 1 : 1);
  db.prepare(`INSERT INTO process_nodes (app_id, node_type, node_order, handler_id, handler_role, handler_name, status, start_time, version) VALUES (?, ?, ?, ?, ?, ?, ?, datetime("now","localtime"), ?)`).run(appId, nodeType, o, handler ? handler.id : null, handler ? handler.role : null, handler ? handler.name : null, status || 'processing', version);
}
...
router.post('/applications/:id/action', (req, res) => {
  ...
  if (!validation.ok) {
    logOperation({ app_id: id, user_id: user.id, user_name: user.name, user_role: user.role, action: action + '_fail', old_status: app.status, new_status: app.status, opinion, reject_reason, version_from: app.version, version_to: app.version, ip: req.ip, extra: JSON.stringify({ reason: validation.msg }) });
  ...
  let isOverdue = app.is_overdue;
  let hasConflict = app.has_conflict;
  if (action === 'audit_pass' && app.status === 'overdue') isOverdue = 0;
  if (action === 'review_archive' && app.status === 'conflict') hasConflict = 0;
  ...
  updateProcessNode(id, mapping.nodeType, oldVersion, user, opinion, mapping.result);
  if (['review_pass', 'review_archive'].includes(action)) {
  } else {
    const nextNodeType = {
    ...
    if (nextNodeType) {
      const nextHandler = db.prepare('SELECT * FROM users WHERE id = ?').get(mapping.nextHandler);
      addProcessNode(id, nextNodeType, null, newVersion, nextHandler, 'processing');
    }
  }
  logOperation({ app_id: id, user_id: user.id, user_name: user.name, user_role: user.role, action, old_status: oldStatus, new_status: mapping.newStatus, opinion, reject_reason, evidence_check: evCheck.status, version_from: oldVersion, version_to: newVersion, ip: req.ip });
  const updated = db.prepare('SELECT * FROM credit_applications WHERE id = ?').get(id);
