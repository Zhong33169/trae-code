    ...
    while let Some(item) = payload.next().await {
        ...
        let result = conn.execute(
            "INSERT INTO ticket_attachments (ticket_id, filename, file_path, file_size, uploaded_by)
             VALUES (?1, ?2, ?3, ?4, ?5)",
            rusqlite::params![ticket_id, filename, file_path, file_size, auth_user.user_id],
        );
...
pub async fn delete_attachment(
    ...
    let file_path: String = match conn.query_row(
        ...
        rusqlite::params![attachment_id, ticket_id],
        |row| row.get(0),
    ) {
        Ok(p) => p,
    ...
    match result {
        Ok(_) => {
            ...
            let conn = db.conn.lock().unwrap();
            let _ = conn.execute(
                "INSERT INTO audit_logs (ticket_id, user_id, action, detail) VALUES (?1, ?2, ?3, ?4)",
                rusqlite::params![ticket_id, auth_user.user_id, "delete_attachment", format!("删除附件: {}", file_path)],
            );
