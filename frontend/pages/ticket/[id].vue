    ...
    <div class="grid grid-cols-3 gap-6">
      <div class="col-span-2 space-y-6">
        ...
        <div v-if="ticket.reject_reason" class="bg-orange-50 border border-orange-200 rounded-lg p-6">
          <h3 class="text-lg font-semibold text-orange-900 mb-2">退回原因</h3>
        ...
        <div v-if="ticket.result" class="bg-green-50 border border-green-200 rounded-lg p-6">
          <h3 class="text-lg font-semibold text-green-900 mb-2">处理结果</h3>
        ...
        <div v-if="ticket.audit_remark" class="bg-blue-50 border border-blue-200 rounded-lg p-6">
          <h3 class="text-lg font-semibold text-blue-900 mb-2">审计备注</h3>
          <p class="text-blue-800 text-sm">{{ ticket.audit_remark }}</p>
        </div>

        <div class="bg-white rounded-lg shadow p-6">
          <h3 class="text-lg font-semibold text-gray-900 mb-4">附件</h3>
      ...
      <div class="bg-white rounded-lg shadow-xl w-full max-w-lg mx-4">
        ...
        <div class="p-4 space-y-4 max-h-96 overflow-y-auto">
          ...
          <div>
            ...
            <textarea v-model="editForm.result" rows="3" class="w-full border border-gray-300 rounded px-3 py-2 text-sm" />
          </div>
        </div>
        <div class="p-4 border-t border-gray-200 flex justify-end gap-3">
...
const canReviewPass = computed(() => {
  if (!ticket.value || !isAuditor.value) return false;
  return ticket.value.stage === 'product_review' && ['in_review', 'review_passed'].includes(ticket.value.status);
});
...
const canEdit = computed(() => {
  ...
  if (isAuditor.value) return true;
  return false;
});
