...
type OperationEvent = OperationRecord & {
  type: 'operation';
  sortTime: string;
};

type RiskEvent = RiskLevelLog & {
  type: 'risk';
  sortTime: string;
};

type TimelineEvent = OperationEvent | RiskEvent;
...
function isOperationEvent(event: TimelineEvent): event is OperationEvent {
...
export default function OperationTimeline({ operations, riskLogs }: OperationTimelineProps) {
  const allEvents: TimelineEvent[] = [
    ...operations.map((op): OperationEvent => ({
      ...op,
      type: 'operation',
      sortTime: op.created_at,
    })),
    ...riskLogs.map((log): RiskEvent => ({
      ...log,
      type: 'risk',
            ...
            <div key={`${event.type}-${event.id}`} className="relative pl-10">
              ...
              <div className={`rounded-lg p-3 border ${
                ...
                <div className="text-xs text-gray-600 mb-2">
                  ...
                  {isOperationEvent(event) && event.version_before !== undefined && (
                    <span className="ml-3 font-mono bg-gray-200 px-1.5 py-0.5 rounded text-gray-700">
                      v{event.version_before} → v{event.version_after}
                    </span>
                  )}
                </div>
                ...
                {isOperationEvent(event) && event.remark && (
                  <div className="text-sm text-gray-700 mt-2 pt-2 border-t border-gray-200">
                    <span className="font-medium">备注：</span>
                    {event.remark}
                  </div>
                )}
                {event.type === 'risk' && event.change_reason && (
                  <div className="text-sm text-gray-700 mt-2 pt-2 border-t border-gray-200">
                    <span className="font-medium">变更原因：</span>
                    {event.change_reason}
                  </div>
                )}
                {isOperationEvent(event) && event.evidence_checked && (
