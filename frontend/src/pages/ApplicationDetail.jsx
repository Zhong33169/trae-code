...
function ApplicationDetail() {
  ...
  const [archiveForm, setArchiveForm] = useState({ review_comment: '', audit_remark: '', force_archive: false });
  ...
  const handleArchive = async (e) => {
    ...
    try {
      ...
      setArchiveForm({ review_comment: '', audit_remark: '', force_archive: false });
      setValidationErrors(null);
      loadData();
    } catch (err) {
      if (err.status === 422 && err.data?.validation_errors) {
  ...
  const handleForceArchive = async () => {
    try {
      await api.archiveApplication(id, { ...archiveForm, force_archive: true });
      setShowArchiveModal(false);
      setArchiveForm({ review_comment: '', audit_remark: '', force_archive: false });
      setValidationErrors(null);
      loadData();
  ...
  return (
    <div>
      ...
      {showArchiveModal && (
        <div className="modal-overlay" onClick={() => { setShowArchiveModal(false); setValidationErrors(null); }}>
          <div className="modal" onClick={e => e.stopPropagation()}>
            <h3>复核归档</h3>
            {validationErrors && (
              <div className="alert alert-error">
                <strong>校验失败：</strong>
                <div>{validationErrors.validation_errors}</div>
              </div>
            )}
            <form onSubmit={handleArchive}>
              ...
              <div className="modal-actions">
                <button type="button" className="btn btn-outline" onClick={() => { setShowArchiveModal(false); setValidationErrors(null); }}>
                  取消
                </button>
                {validationErrors?.can_force && (
                  <button type="button" className="btn btn-warning" onClick={handleForceArchive}>
                    强制归档
                  </button>
                )}
