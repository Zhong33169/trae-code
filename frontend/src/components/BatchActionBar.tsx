      ...
      {results && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl w-full max-w-md max-h-[80vh] overflow-y-auto">
            <div className="p-6">
              <h3 className="text-lg font-bold text-navy mb-4">批量操作结果</h3>
              <div className="space-y-2">
                {results.map((r) => (
                  ...
                  >
                    ...
                    {!r.success && r.error && <span className="text-xs ml-auto">{r.error}</span>}
                  </div>
                ))}
              </div>
              <button
                onClick={closeResults}
                className="mt-4 w-full bg-navy text-white py-2 rounded-md text-sm font-medium hover:bg-navy/90"
              >
                关闭
              </button>
            </div>
