...
let TopicDetailComponent = class TopicDetailComponent {
    constructor(auth, service, route, router) {
        this.auth = auth;
        this.service = service;
        this.route = route;
        this.router = router;
        this.topic = signal(null);
        ...
        this.rectify = { topic_no: '', title: '', source: '', reporter: '', department: '', deadline: '', content: '' };
    }
    ngOnInit() {
    ...
    canDoAnything() {
        ...
        if (!t)
            return false;
        if (t.status === 'registered' && this.auth.hasRole(['reviewer']))
            return true;
        if (t.status === 'reviewed' && this.auth.hasRole(['archiver']))
            return true;
        if (t.status === 'rejected' && this.auth.hasRole(['registrar']))
            return true;
        return false;
    }
    statusLabel(s) {
        return { registered: '已登记', reviewed: '已审核', archived: '已归档', rejected: '已退回' }[s || ''] || s;
    }
    anomalyLabel(a) {
        return { normal: '正常', missing_material: '缺材料', overdue: '超时', rejected: '退回' }[a || 'normal'] || '正常';
    }
    actionLabel(a) {
        return {
            create: '登记',
